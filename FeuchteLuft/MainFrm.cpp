...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndProcessToolBar.CreateEx(this, TBSTYLE_FLAT, WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) ||
		!m_wndProcessToolBar.LoadToolBar(IDR_PROCESS))
	{
		TRACE0("Fehler beim Erstellen der Prozess-Symbolleiste.\n");
		return -1;      // Fehler beim Erstellen
	}
	...
	CString strProcessToolBarName;
	bNameValid = strProcessToolBarName.LoadString(IDS_TOOLBAR_PROCESS);
	ASSERT(bNameValid);
	m_wndProcessToolBar.SetWindowText(strProcessToolBarName);
