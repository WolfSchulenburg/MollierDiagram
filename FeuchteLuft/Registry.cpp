...
CStringList* CRegistry::GetStringList(CString sKey)
{
	...
	CString sItems = CRegistry::GetString(sKey, _T(""));
...
{
	CSize szReturn;
	CString sSize = GetString(sKey, _T(""));
	int nFoundComma = -1;
...
void CRegistry::SetPrinterMode(HANDLE DevMode, HANDLE PrinterName)
{
	...
	if (theDevMode != NULL)
	{
		Set(_T("PrinterMode"), (void *)theDevMode, theDevMode->dmSize + theDevMode->dmDriverExtra);
	}
