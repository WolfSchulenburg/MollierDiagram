...
IMPLEMENT_DYNCREATE(CMollierCtrl, CWnd)

// construct 
CMollierCtrl::CMollierCtrl()
{
	RegisterWindowClass();
...
BEGIN_MESSAGE_MAP(CMollierCtrl, CWnd)
...
//               nID        -- resource ID
//               dwStyle    -- style
BOOL CMollierCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	ASSERT(pParentWnd->GetSafeHwnd());

	if (!CWnd::Create(MOLLIERCTRL_CLASSNAME, NULL, dwStyle, rect, pParentWnd, nID))
		return FALSE;

	//Add ToolTips
	if (!m_Tooltip.Create(this))
		TRACE(_T("Unable to create tip window for CMollierCtrl."));
	else if (!m_Tooltip.AddTool(this, _T("Control ToolTips")))
		TRACE(_T("Unable to add tip for the control window for CMollierCtrl."));
	else
...
BOOL CMollierCtrl::RegisterWindowClass()
{
	WNDCLASS wndcls;
	HINSTANCE hInst = AfxGetInstanceHandle();

	if (!(::GetClassInfo(hInst, MOLLIERCTRL_CLASSNAME, &wndcls)))
	{
		...
		wndcls.lpszClassName = MOLLIERCTRL_CLASSNAME;
...
//               rect -- paint area
void CMollierCtrl::DrawGrid(CDC *pdc, const CRect& rect)
{
...
void CMollierCtrl::DrawCurve(CDC *pdc)
{
	CPen		Pen;
	CBrush		brush;
	CPen*		pOldPen;
...
int	CMollierCtrl::AddCurve(const CString& strName, COLORREF color, int iStyle, int nWidth)
{
	if (strName.IsEmpty())
		return -1;
	for (int iCurve = 0; iCurve < m_ArrCurve.GetSize(); iCurve++)
...
BOOL CMollierCtrl::AddData(const CString& strName, float fHoriValue, float fVertValue)
{
	if (strName.IsEmpty())
		return FALSE;
...
BOOL CMollierCtrl::AddData(CCurve* pCurve, float fHori, float fVert)
{
	if (!pCurve)
		return FALSE;
...
int CMollierCtrl::InsertDataToCurve(CCurve* pCurve, float fHori, float fVert, CPoint point)
{
	if (!pCurve)
		return -1;
	ASSERT(pCurve->m_fArrHoriValue.GetSize() == pCurve->m_fArrVertValue.GetSize());
...
BOOL CMollierCtrl::AddCurveData(const CString& strName, const CArray< float, float >& ArrHori, const CArray< float, float >& ArrVert)
{
...
void CMollierCtrl::SortCurveData(CCurve* pCurve)
{
...
CCurve* CMollierCtrl::GetCurve(int iIndex)
{
...
CCurve* CMollierCtrl::GetCurve(const CString& strName)
{
	CCurve*		pCurve = NULL;
	for (int iCurve = 0; iCurve < m_ArrCurve.GetSize(); iCurve++)
	{
...
int	CMollierCtrl::GetIndex(const CCurve* pCurve)
{
	int iPos = -1;
...
BOOL CMollierCtrl::Remove(int index)
{
...
BOOL CMollierCtrl::Remove(const CString& strName)
{
	for (int iCurve = 0; iCurve < m_ArrCurve.GetSize(); iCurve++)
	{
		if (strName == m_ArrCurve[iCurve]->m_strName)
...
int	CMollierCtrl::GetCurveCount()
{
	return m_ArrCurve.GetSize();
}

// decription  : get selected curve count
int	CMollierCtrl::GetSelectedCount()
{
	int nSelected = 0;
...
BOOL CMollierCtrl::CalculatePoint(float fHori, float fVert, CPoint& point)
{
	point = INVALID_POINT;
...
BOOL CMollierCtrl::CalculateValue(const CPoint& point, float& fHori, float& fVert)
{
	if (!m_ArrCurve.GetSize())
		return FALSE;
...
void CMollierCtrl::ReCalcAllPoint()
{
...
void CMollierCtrl::DrawCross(CDC *pdc)
{
	CPoint point;
	GetCursorPos(&point);
...
BOOL CMollierCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
...
void CMollierCtrl::Move(BOOL bLeft)
{
	int nHoriGrid = 10;
...
void CMollierCtrl::MovePage(BOOL bLeft)
{
...
//               fLen   -- offset
void CMollierCtrl::CurveRight(CCurve* pCurve, float fLen)
{
	if (pCurve == NULL)
		return;
...
// in parameter: pCurve -- curve object
//               fLen   -- offset
void CMollierCtrl::CurveLeft(CCurve* pCurve, float fLen)
{
	if (pCurve == NULL)
		return;
...
BOOL CMollierCtrl::Zoom(BOOL bIn)
{
	bIn ? m_iZoom++ : m_iZoom--;
	if (m_iZoom == 4)
	{
...
// in parameter: pCurve -- curve object
//               fMid   -- center value 
void CMollierCtrl::MirrorHori(CCurve* pCurve, float fMid)
{
	if (!pCurve)
		return;
...
// in parameter: pCurve -- curve object
//               fMid   -- center value 
void CMollierCtrl::MirrorVert(CCurve* pCurve, float fMid)
{
	if (!pCurve)
		return;
...
void CMollierCtrl::CalculateVertRange(float fValue, BOOL bMax)
{
	CString str;
...
void CMollierCtrl::SetHoriLabel(CString& str)
{
	m_strHoriLabel = str;
}

CString CMollierCtrl::GetHoriLabel()
{
	return m_strHoriLabel;
}

void CMollierCtrl::SetVertLabel(CString& str)
{
	m_strVertLabel = str;
}

CString CMollierCtrl::GetVertLabel()
{
	return m_strVertLabel;
}

void CMollierCtrl::ShowCross(BOOL bShow)
{
	m_bShowCross = bShow;
}

BOOL CMollierCtrl::IsShowCross()
{
	return m_bShowCross;
}
...
void CMollierCtrl::SetMargin(const CRect& rect)
{
	m_Margin = rect;
}

CRect CMollierCtrl::GetMargin()
{
	return m_Margin;
}

// enable/disable edit by using mouse key operation
void CMollierCtrl::EnableEdit(BOOL bEdit)
{
...
BOOL CMollierCtrl::CanEditCurve()
{
	return m_bEdit;
}

void CMollierCtrl::SetGridLineStyle(int iStyle)
{
	m_iGridLineStyle = iStyle;
}

int	 CMollierCtrl::GetGridLineStyle()
{
...
//                
void CMollierCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	// disable tooltip	
	if (m_Tooltip.m_hWnd)
...
void CMollierCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CMollierCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_bEdit && m_pCurveEdit && m_iCurPoint >= 0)
	{
...
void CMollierCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (m_bEdit)
	{
...
LRESULT CMollierCtrl::SendMessageToParent(int nMessage, CCurve* pCurve, int iIndex, float fHori, float fVert) const
{
	if (!IsWindow(m_hWnd))
		return 0;
